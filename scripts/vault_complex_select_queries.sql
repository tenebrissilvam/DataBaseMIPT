...
select 
	...
	a.pet_name, a.fighting_strength
from cd.person p 
	right join cd.animal a 
	on p.pet_id = a.anim_id
...
from cd.person p 
	join cd.task_history th 
	on p.person_id = th.executor
where th.task_ended >= '1999-01-10 00:00:00' 
	and th.task_ended < '1999-01-16 00:00:00'
...
select 
	p.firstname ||' '|| p.surname as person,
	date_part('hour', ss.sleep_ended - ss.sleep_started) * b.res_stamina  as restored,
	avg(date_part('hour', ss.sleep_ended - ss.sleep_started) * b.res_stamina) 
	over(partition by p.responsibility) as average
from cd.person p 
	join sleep_schedule ss 
	on p.person_id = ss.person_id 
	join cd.bedroom b 
	on ss.room_id = b.bedroom_id
...
select 
	sub.responsibility, 
	sub.prov_type as top_food, 
	sub.common_food as counter
from (
	select 
		ps.responsibility, pr.prov_type, count(*) as common_food, row_number()
		over (partition by ps.responsibility order by count(*) desc) as row_num 
	from cd.person ps 
		join cd.provisions pr 
		on ps.consuming_prov = pr.prov_id
...
select 
	t.task_type, a.appl_type, 
	rank() over (order by stamina_req desc, task_type) as rank 
from cd.task t 
join cd.appliance a on t.appl_id = a.appl_id 
group by t.task_type, a.stamina_req, a.appl_type;

--Сравнение количества восстановленных сил с предыдущим спящим и следующим спящим

select p.firstname ||' '|| p.surname as person,
	res_stamina * date_part('hour', ss.sleep_ended - ss.sleep_started) 
	as restored,
	lag(res_stamina * date_part('hour', ss.sleep_ended - ss.sleep_started)) 
	over (order by (ss.sleep_ended - ss.sleep_started))
	as next_person_restored,
	lead(res_stamina * date_part('hour', ss.sleep_ended - ss.sleep_started)) 
	over (order by (ss.sleep_ended - ss.sleep_started))
	as previous_person_restored	
from cd.person p join cd.sleep_schedule ss 
on ss.person_id = p.person_id
join cd.bedroom b on b.bedroom_id = ss.room_id;
